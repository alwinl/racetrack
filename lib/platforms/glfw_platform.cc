/*
 * glfw_platform.cc Copyright 2026 Alwin Leerling dna.leerling@gmail.com
 *
...
bool GLFWPlatform::create_window( InputQueue &sink )
{
    if( !glfwInit() )
        return false;
	...
	{
		GLFWPlatform * self = static_cast<GLFWPlatform*>( glfwGetWindowUserPointer( window ) );
		self->handle_key( key, scancode, action, mods );
	});
	...
	{
		GLFWPlatform * self = static_cast<GLFWPlatform*>( glfwGetWindowUserPointer( window ) );
		self->handle_mouse_button( button, action, mods );
	});

	return true;
}

void GLFWPlatform::destroy_window()
{
    if( window )
...
void GLFWPlatform::begin_render()
{
}

void GLFWPlatform::present_frame()
{
...
bool GLFWPlatform::should_close()
{
	return glfwWindowShouldClose(window);
}

void GLFWPlatform::handle_key( int key, int scancode, int action, int mods )
{
	if( !event_sink )
		return;

	if( (action == GLFW_PRESS) || (action == GLFW_REPEAT) )
		event_sink->push( std::make_unique<KeyPressEvent>( key_mapping[key], scancode, mods) );
	else
		event_sink->push( std::make_unique<KeyReleaseEvent>( key_mapping[key], scancode, mods) );
}

void GLFWPlatform::handle_mouse_move( double xpos, double ypos )
{
	if( !event_sink )
		return;

	event_sink->push( std::make_unique<MouseMoveEvent>(xpos, ypos) );
}

void GLFWPlatform::handle_mouse_button( int button, int action, int mods )
{
	if( !event_sink )
		return;
	...
	event_sink->push( std::make_unique<MouseButtonEvent>(xpos, ypos, button, action, mods) );
}
